...
function parseArgs() {
    ...
    const mode = (modeIndex !== -1 && process.argv[modeIndex + 1]) || 'test-user';
    if (!tenant) {
...
let tenantIdGlobal = '';
let modeGlobal = 'test-user';
let tokenRefreshInterval = null;
// Subscribe to events
async function subscribeToEvents(supabase, tenantId) {
...
async function handleTokenExpiration() {
    try {
        ...
        const newSupabase = await (0, test_auth_1.createAuthenticatedClient)(tenantIdGlobal, modeGlobal);
...
async function sendCreateOrderCommand(supabase, tenantId) {
    try {
        ...
        const { error, data } = await supabase.from('infra.commands').insert(command).select();
        if (error) {
...
async function main() {
    try {
        ...
        const supabase = await (0, test_auth_1.createAuthenticatedClient)(tenant, mode);
        // Setup token refresh interval
        setupTokenRefreshInterval();
        // Subscribe to events
        ...
        const heartbeatInterval = setInterval(async () => {
            try {
                ...
                const { error } = await supabaseClient.from('infra.events').select('id').limit(1);
                if (error) {

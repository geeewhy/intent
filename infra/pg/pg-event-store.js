...
exports.PgEventStore = exports.SNAPSHOT_EVERY = void 0;
const pg_1 = require("pg");
const logger_1 = require("../../core/logger");
const event_upcaster_1 = require("../../core/shared/event-upcaster");
exports.SNAPSHOT_EVERY = 2; // Number of events after which to take a snapshot
/**
...
class PgEventStore {
     ...
     */
    async snapshotAggregate(tenantId, aggregate) {
     ...
     */
    async append(tenantId, aggregateType, aggregateId, events, expectedVersion, snapshot) {
        ...
        try {
         ...
         FOR
                UPDATE`, [tenantId, aggregateId]);
            const dbVersion = rows[0]?.version ?? 0;
            if (dbVersion !== expectedVersion) {
                throw new Error(`Concurrency error: expected version ${expectedVersion}, got ${dbVersion}`);
            }
            // Insert events
            let numberEvents = 0;
            for (numberEvents = 0; numberEvents < events.length; numberEvents++) {
                ...
                await client.query(`INSERT INTO infra.events (tenant_id, id, aggregate_id, aggregate_type,
                ...
                await client.query(`NOTIFY "events:${tenantId}", '${payload}'`);
            }
            // Persist snapshot (if provided) atomically
            const crossedSnapshotThreshold = snapshot &&
                Math.floor((expectedVersion + numberEvents) / exports.SNAPSHOT_EVERY) >
                    Math.floor(expectedVersion / exports.SNAPSHOT_EVERY);
            if (crossedSnapshotThreshold) {
                await client.query(`INSERT INTO infra.aggregates (id, tenant_id, type, version, snapshot, schema_version, created_at)
        ...
        try {
            ...
            const events = eventsResult.rows.map(row => {
                ...
                return {
                    ...
                    version: row.version,
                    payload: (0, event_upcaster_1.upcastEvent)(row.type, row.payload, schemaVersion),
...
exports.PgEventStore = PgEventStore;
//# sourceMappingURL=pg-event-store.js.map
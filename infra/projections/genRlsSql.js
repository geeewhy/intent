...
async function generateRlsPolicies() {
    ...
    const readAccessFiles = (0, glob_1.globSync)('src/core/**/read-models/read-access.ts');
    ...
    for (const filePath of readAccessFiles) {
        try {
            ...
            const readModelPolicies = module.ReadModelPolicies;
            // Process each policy
            for (const [scopeName, policy] of Object.entries(readModelPolicies)) {
                ...
                sqlCondition = sqlCondition.replace(/current_setting\('request\.jwt\.claims', true\)::json->>'([^']+)'\s*(=|!=|<>|>|<|>=|<=)\s*([^:\s]+)/g, "current_setting('request.jwt.claims', true)::json->>'$1' $2 $3::text");
                // Handle cases where JWT claim is on the right side of the comparison with various operators
                sqlCondition = sqlCondition.replace(/([^:\s]+)\s*(=|!=|<>|>|<|>=|<=)\s*current_setting\('request\.jwt\.claims', true\)::json->>'([^']+)'/g, "$1::text $2 current_setting('request.jwt.claims', true)::json->>'$3'");
                ...
                sqlCondition = sqlCondition.replace(/(\w+_id)(?!::text)(\s|$|\))/g, "$1::text$2");
                // Handle regular identifiers that are exactly 'id'
                sqlCondition = sqlCondition.replace(/\bid\b(?!::text)(\s|$|\))/g, "id::text$1");
                // Handle quoted identifiers that end with Id (like "testerId", "userId")
                sqlCondition = sqlCondition.replace(/("(\w+Id)")(?!::text)(\s|$|\))/g, "$1::text$3");
                // Handle quoted identifier that is exactly "id"
                sqlCondition = sqlCondition.replace(/("id")(?!::text)(\s|$|\))/g, "$1::text$2");
...
//# sourceMappingURL=genRlsSql.js.map
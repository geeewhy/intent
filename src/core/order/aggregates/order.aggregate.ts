...
export class OrderAggregate extends BaseAggregate<OrderSnapshotState> {
    ...
    public static rehydrate(events: Event[]): OrderAggregate {
        ...
        const base = new OrderAggregate(events[0].aggregateId);
    ...
    public apply(event: Event, isNew: boolean = true): void {
        ...
        if (isNew) {
            ...
            console.log("new event applied, version incremented", event.type, event.payload?.numberExecutedTests, event.id, event.version, this.version);
        } else {
    ...
    protected applyUpcastedSnapshot(state: OrderSnapshotState): void {
        Object.assign(this, {
            ...
            status: state.status,
            numberExecutedTests: state.numberExecutedTests,

//src/tools/setup/flows/eventstore/providers/postgres/steps/reset.ts
/**
...
export default async function step(ctx: FlowCtx): Promise<void> {
    ...
    if (!hasYesFlag) {
        const confirmed = await promptYesNo(
            'WARNING: This will permanently delete all data in the event store tables. Continue?',
            false
        );
    ...
    try {
        ...
        ctx.logger.error(`Failed to drop tables: ${(error as Error).message}`);
        throw error;
    }
}
 ...
 */
async function checkTablesExist(pool: pg.Pool): Promise<string[]> {

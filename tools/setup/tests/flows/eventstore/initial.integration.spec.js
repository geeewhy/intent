...
const mockDbStep = jest.fn(async (ctx) => {
    ...
    ctx.vars.executeQuery = async (query, params = []) => {
...
jest.mock(process.cwd() + '/src/tools/setup/flows/eventstore/providers/postgres/steps/connection', () => ({
    default: mockDbStep
}), { virtual: true });
jest.mock(process.cwd() + '/src/tools/setup/flows/eventstore/providers/postgres/steps/schema', () => ({
...
jest.mock(process.cwd() + '/src/tools/setup/flows/eventstore/providers/postgres/steps/test', () => ({
...
describe('integration – eventstore initial', () => {
    ...
    it('migrates schema and performs smoke RW', async () => {
        // WHEN
        await (0, runner_1.runFlow)('eventstore', { provider: 'postgres', path: 'initial', yes: true });
    ...
    `);
        ...
        await executeQuery(`INSERT INTO events(id, stream_id, type, version, payload) VALUES ($1, $2, $3, $4, $5)`, [testEvent.id, testEvent.stream_id, testEvent.type, testEvent.version, testEvent.payload]);
        ...
        await executeQuery(`INSERT INTO snapshots(stream_id, version, state) VALUES ($1, $2, $3)`, [testEvent.stream_id, testEvent.version, snapshotState]);
        const snapshotOut = await executeQuery(`SELECT * FROM snapshots WHERE stream_id = $1`, [testEvent.stream_id]);
...
//# sourceMappingURL=initial.integration.spec.js.map